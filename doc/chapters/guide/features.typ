...
Let's say you're writing a book, this means that for a reader, there are always two page visible when reading, the _trailing_ and the _leading_ page.
...
This may produce unexpected results if @cmd:hydra is used outside the header and the text direction where it is used is different to where its anchor (see @sec:anchor) is placed.
This can be prevented by explicitly setting #arg("binding") and #arg("dir").
