...
///
/// -> direction
#let get-text-dir() = util.auto-or(
  text.dir,
  () => util.text-direction(text.lang)
)
...
///
/// -> alignment
#let get-page-binding() = util.auto-or(
  page.binding,
  () => util.page-binding(get-text-dir()),
)
...
#let is-active-visible(
  ...
  // depending on the reading direction and binding combination the leading page
  // is either on an odd or even number, if it is leading it means the previous
  // page is visible
  let cases = (
    left: (
      ltr: calc.odd,
